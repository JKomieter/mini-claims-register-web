...
import { Claim, Total } from "@/types";
import { format } from "date-fns";
import { useState } from "react";
import ViewEditClaimModal from "./ClaimDetails";
...
export default function ClaimsTable({
  ...
  const [isViewClaimModalOpen, setIsViewClaimModalOpen] = useState(false);
  ...
  return (
    <>
                ...
                claims.map((claim) => {
                  ...
                  return (
                    <TableRow key={claim.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        <span
                          className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${statusBadgeStyles[statusType]
                            }`}
                        >
          ...
          <div className="border-t border-border bg-muted/20">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {totals?.map((total) => (
                    <tr key={total.currency} className="border-b border-border/50 hover:bg-muted/30">
                      ...
                      <td className="px-6 py-3 text-right">
                        ...
                        <div className="font-medium">
                          {formatCurrency(total.total_estimated, total.currency)}
                        </div>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <div className="text-xs text-muted-foreground">Approved</div>
                        <div className="font-medium">
                          {formatCurrency(total.total_approved, total.currency)}
                        </div>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <div className="text-xs text-muted-foreground">Total Paid</div>
                        <div className="font-medium text-green-600 dark:text-green-400">
                          {formatCurrency(total.total_paid, total.currency)}
                        </div>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <div className="text-xs text-muted-foreground">Outstanding</div>
                        <div className="font-bold text-red-600 dark:text-red-400">
                          {formatCurrency(total.total_outstanding, total.currency)}
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <ViewEditClaimModal
        ...
        open={isViewClaimModalOpen}
        onOpenChange={setIsViewClaimModalOpen}
      />
    </>
  );
}